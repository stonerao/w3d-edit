...
// Mesh头上绑定字体
class CreatBoxTitle extends EffectBase {
    constructor(config) {
        super(config);
    ...
    add(option, obj) {
        ...
        const position = new THREE.Vector3(
            THREE.MathUtils.lerp(max.x, min.x, 0.5),
            THREE.MathUtils.lerp(max.y, min.y, 0.5),
            THREE.MathUtils.lerp(max.z, min.z, 0.0) + option.offsetY,
        )
        // 判断当前obj下是否已有boxLine
        this.remove(obj);
    ...
    remove(mesh) {
        const obj = mesh.getObjectByProperty("name", "BoxTitle");
        if (obj) {
